...
import { useRouter } from "next/router"; // Import the appropriate routing hook for Next.js
import {
...
import FlashCard from "../components/FlashCard"; // Update the path to the FlashCard component

const InputChatGPT = () => {
  const [input, setInput] = useState("");
  const [output, setOutput] = useState("");
  const [searchHistory, setSearchHistory] = useState([]);
  const router = useRouter(); // Use the useRouter hook for routing in Next.js
  ...
  const fetchSearchHistory = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/users/search-history`, // Update the environment variable usage for the base URL
        {
          method: "GET",
          headers: {
  ...
  const handleClick = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/users/chat`, // Update the environment variable usage for the base URL
        {
          method: "POST",
  ...
  const restoreSearch = async (searchId) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/users/search-history/${searchId}`, // Update the environment variable usage for the base URL
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
